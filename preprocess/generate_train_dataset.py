...
class TrainDatasetGenerator:
    ...
    def _select_processing_crs(self, first_crs: CRS, first_bounds: Tuple[float, float, float, float]) -> CRS:
        if self.args.processing_crs:
            user_crs = CRS.from_user_input(self.args.processing_crs)
            if self._is_metric_projected_crs(user_crs):
                return user_crs
            print(
                f"[Warn] User provided processing CRS ({user_crs}) is not meter-based. "
                "Falling back to auto-estimated UTM CRS to keep window_size_m/grid_step_m physically correct."
            )
            center_lon, center_lat = self._get_center_lonlat(first_bounds, first_crs)
            return self._utm_crs_from_lonlat(center_lon, center_lat)

        if first_crs.is_projected and self._is_metric_projected_crs(first_crs):
            return first_crs

        center_lon, center_lat = self._get_center_lonlat(first_bounds, first_crs)
        if first_crs.is_projected:
            print(
                f"[Warn] First image CRS ({first_crs}) is projected but non-metric. "
                "Using auto-estimated UTM CRS for meter-denominated window parameters."
            )
        return self._utm_crs_from_lonlat(center_lon, center_lat)

    def _utm_crs_from_lonlat(self, lon: float, lat: float) -> CRS:
        zone = int((lon + 180) // 6) + 1
        epsg = 32600 + zone if lat >= 0 else 32700 + zone
        return CRS.from_epsg(epsg)

    def _get_center_lonlat(self, bounds: Tuple[float, float, float, float], src_crs: CRS) -> Tuple[float, float]:
        minx, miny, maxx, maxy = bounds
        cx = (minx + maxx) / 2.0
        cy = (miny + maxy) / 2.0
        if src_crs.is_geographic:
            return cx, cy
        lon, lat = transform(src_crs, CRS.from_epsg(4326), [cx], [cy])
        return float(lon[0]), float(lat[0])

    @staticmethod
    def _is_metric_projected_crs(crs: CRS) -> bool:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--window_size_m", type=float, default=1500, help="Crop window size in meters.")
    parser.add_argument("--output_size_px", type=int, default=3000, help="Output crop size in pixels (square).")
    ...
    parser.add_argument("--coverage_threshold", type=float, default=1.0, help="Area coverage ratio threshold per image.")
