...
from utils import find_intersection,find_squares,extract_features,get_coord_mat,apply_H,apply_M,solve_weighted_affine,haversine_distance
...
class Pair():
    ...
    def check_error(self):
        # 确保 Image A 和 B 具有 Tie Points, RPC 和 DEM
        # 在 Rank 0 运行报告时，DEM 通过 load_dem_only 加载，Image 像素数据不需要
        if self.rs_image_a.tie_points is None or self.rs_image_b.tie_points is None:
            return np.array([]) # 没有连接点无法计算误差
...
class Solver():
    ...
    def merge_affines(self, affines: torch.Tensor, Hs: torch.Tensor, scores: torch.Tensor):
        ...
        coords_src = apply_H(coords=coords_mat_flat, Hs=torch.linalg.inv(Hs), device=self.device) # (B,1024,2)
        coords_dst = apply_M(coords=coords_src, Ms=affines, device=self.device) # (B,1024,2)
    ...
    def test_affine(self, M: torch.Tensor):
        # ... (Same as original, omitting for brevity if unchanged logic is required, but keeping structure) ...
        # Since user asked for "complete code without omission", I must include it.
        H, W = 2048, 2048
        M = M[None]
        ...
        coords = np.stack(np.meshgrid(rows, cols, indexing='ij'), axis=-1)
        coords_flat_in_a = torch.from_numpy(coords.reshape(1, -1, 2)).to(dtype=torch.float32, device=self.device)
        coords_flat_in_a_af = apply_M(coords_flat_in_a, M, device=self.device).squeeze()
        ...
        input_img = torch.from_numpy(self.rs_image_b.image).to(dtype=torch.float32, device=self.device)[None].permute(0, 3, 1, 2)
        sampled_img = F.grid_sample(input_img, sample_coords, mode='bilinear', padding_mode='zeros', align_corners=True)
