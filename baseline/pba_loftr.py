...
def x84_filter_points(pts_a: np.ndarray, pts_b: np.ndarray, k: float, min_points: int):
    if pts_a.shape[0] < min_points or pts_b.shape[0] < min_points:
        return np.empty((0, 2)), np.empty((0, 2))
    M, _ = cv2.estimateAffine2D(pts_a, pts_b, method=cv2.RANSAC, ransacReprojThreshold=3.0)
    if M is None:
        return np.empty((0, 2)), np.empty((0, 2))
    pts_a_h = np.concatenate([pts_a, np.ones((pts_a.shape[0], 1), dtype=pts_a.dtype)], axis=1)
    proj = (M @ pts_a_h.T).T
    residuals = np.linalg.norm(proj - pts_b, axis=1)
    median = np.median(residuals)
    mad = np.median(np.abs(residuals - median))
    if mad <= 1e-12:
        return pts_a, pts_b
    sigma = 1.4826 * mad
    keep = np.abs(residuals - median) <= k * sigma
...
def filter_by_confidence(
    ...
    conf_thresh: float,
    conf_reduce: str,
    img_shape_a: Tuple[int, int],
    img_shape_b: Tuple[int, int],
):
    if pts_a.shape[0] == 0:
        return pts_a, pts_b
    conf_a_np = conf_a.squeeze(1).cpu().numpy()
    conf_b_np = conf_b.squeeze(1).cpu().numpy()
    h_a, w_a = conf_a_np.shape[-2:]
    h_b, w_b = conf_b_np.shape[-2:]
    img_h_a, img_w_a = img_shape_a
    img_h_b, img_w_b = img_shape_b
    scale_a_x = w_a / float(img_w_a)
    scale_a_y = h_a / float(img_h_a)
    scale_b_x = w_b / float(img_w_b)
    scale_b_y = h_b / float(img_h_b)
    xs_a = np.clip((pts_a[:, 0] * scale_a_x).round().astype(int), 0, w_a - 1)
    ys_a = np.clip((pts_a[:, 1] * scale_a_y).round().astype(int), 0, h_a - 1)
    xs_b = np.clip((pts_b[:, 0] * scale_b_x).round().astype(int), 0, w_b - 1)
    ys_b = np.clip((pts_b[:, 1] * scale_b_y).round().astype(int), 0, h_b - 1)
    conf_vals_a = conf_a_np[0, ys_a, xs_a]
    conf_vals_b = conf_b_np[0, ys_b, xs_b]
    if conf_reduce == "min":
        conf_vals = np.minimum(conf_vals_a, conf_vals_b)
    elif conf_reduce == "mean":
        conf_vals = (conf_vals_a + conf_vals_b) / 2.0
    else:
        conf_vals = conf_vals_a
    keep = conf_vals >= conf_thresh
    return pts_a[keep], pts_b[keep]


def build_match_points(args, image_a: RSImage, image_b: RSImage, matcher, reporter, encoder: Optional[Encoder]):
    ...
    for idx in range(len(imgs_a)):
        ...
        if len(pts_a_crop) == 0:
            continue
        if args.loftr_filter_mode == "x84":
            pts_a_crop, pts_b_crop = x84_filter_points(pts_a_crop, pts_b_crop, args.x84_k, args.x84_min_points)
            if len(pts_a_crop) == 0:
                continue
        if args.loftr_filter_mode == "conf":
            if encoder is None:
                continue
            conf_a, conf_b = extract_conf_maps(encoder, imgs_a[idx:idx + 1], imgs_b[idx:idx + 1], args.device)
            pts_a_crop, pts_b_crop = filter_by_confidence(
                pts_a_crop,
                pts_b_crop,
                conf_a,
                conf_b,
    ...
    try:
        ...
        if len(pairs_ids) > 0:
            ...
            for idx, (i, j) in enumerate(pairs_ids):
                ...
                match_points = build_match_points(args, image_i, image_j, matcher, reporter, encoder)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--loftr_filter_mode', type=str, default='raw', choices=['raw', 'x84', 'conf'])
    parser.add_argument('--x84_k', type=float, default=3.5)
    parser.add_argument('--x84_min_points', type=int, default=8)
